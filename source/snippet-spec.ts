...
describe("Snippet", function (): void {
  ...
  describe("areEquivalentTypeStrings", () => {
    ...
    it("should ignore whitespace differences", () => {
      expect(
        areEquivalentTypeStrings("Observable<number>", "Observable< number >")
      ).to.be.true;
    });
  ...
  describe("snippet", () => {
    describe("fail", () => {
      ...
      it("should not throw if a matching error occurs", () => {
        ...
        expect(() =>
          snip.fail("a.ts", /is not assignable to type 'string'/)
        ).to.not.throw();
      });
      ...
      it("should throw if a non-matching error occurs", () => {
        ...
        expect(() =>
          snip.fail("a.ts", /is not assignable to type 'number'/)
        ).to.throw();
      });
    ...
    describe("expect", () => {
      describe("toFail", () => {
        ...
        it("should not throw if a matching error occurs", () => {
          ...
          expect(() =>
            snip.expect("a.ts").toFail(/is not assignable to type 'string'/)
          ).to.not.throw();
        });
        ...
        it("should throw if a non-matching error occurs", () => {
          ...
          expect(() =>
            snip.expect("a.ts").toFail(/is not assignable to type 'number'/)
          ).to.throw();
        });
      ...
      describe("toInfer", () => {
        ...
        it("should throw if a variable is not found", () => {
          expect(() => snip.expect("a.ts").toInfer("x", "number")).to.throw(
            /variable 'x' not found/i
          );
          expect(() => snip.expect("b.ts").toInfer("x", "number")).to.throw(
            /variable 'x' not found/i
          );
        });

        it("should throw if a variable has an unexpected type", () => {
          expect(() => snip.expect("a.ts").toInfer("a", "string")).to.throw(
            /expected 'a: number' to be 'string'/i
          );
          expect(() => snip.expect("b.ts").toInfer("b", "string")).to.throw(
            /expected 'b: number' to be 'string'/i
          );
        });

        it("should not throw if a variable has the expected type", () => {
          expect(() =>
            snip.expect("a.ts").toInfer("a", "number")
          ).to.not.throw();
          expect(() =>
    ...
    describe("infer", () => {
      ...
      it("should throw if a variable is not found", () => {
        expect(() => snip.infer("a.ts", "x", "number")).to.throw(
          /variable 'x' not found/i
        );
        expect(() => snip.infer("b.ts", "x", "number")).to.throw(
          /variable 'x' not found/i
        );
      });

      it("should throw if a variable has an unexpected type", () => {
        expect(() => snip.infer("a.ts", "a", "string")).to.throw(
          /expected 'a: number' to be 'string'/i
        );
        expect(() => snip.infer("b.ts", "b", "string")).to.throw(
          /expected 'b: number' to be 'string'/i
        );
      });
